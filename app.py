...
# ── Load model (lazy, inside function to avoid top-level cv2 import) ──────────
@st.cache_resource
def load_model():
    # Import YOLO only when needed — after packages are installed
    from ultralytics import YOLO
    return YOLO("best.pt")

def draw_boxes_pil(image: Image.Image, boxes) -> Image.Image:
    """Draw bounding boxes using PIL only — no cv2 needed."""
    img = image.copy().convert("RGB")
    draw = ImageDraw.Draw(img)
    for box in boxes:
        x1, y1, x2, y2 = [int(v) for v in box.xyxy[0].tolist()]
        draw.rectangle([x1, y1, x2, y2], outline="#86ef5c", width=3)
        draw.rectangle([x1, y1, x1 + 60, y1 + 18], fill="#86ef5c")
        draw.text((x1 + 4, y1 + 2), "Mango", fill="#071a09")
    return img
...
if uploaded_files:
    ...
    for i, f in enumerate(uploaded_files):
        ...
        # Draw boxes using PIL only — no cv2
        annotated = draw_boxes_pil(image, result.boxes)
